...
def print_event(event: Dict[str, Any], show_payload: bool, participant_colors: Dict[str, str], show_queue: bool, truncate_names: bool, event_name_width: int) -> str:
    ...
    if show_queue:
        output += f"{receive_queue_len:<3}  "
    output += f"{colorize_event_name(event_name):<{event_name_width}}  "
...
def print_header(show_queue: bool, show_payload: bool, event_name_width: int) -> str:
    ...
    if show_queue:
        header += f"{'Queue Len':<3}  "
    header += f"{'Event Name':<{event_name_width}}  "
    if show_payload:
        header += f"{'Payload':<50}"
    header += f"{COLORS['RESET']}"
    return header

def main(file_name: str, show_payload: bool, show_participants: List[str], exclude_participants: List[str], show_queue: bool, truncate_names: bool) -> None:
    ...
    output_lines.extend(print_event(event, show_payload, participant_colors, show_queue, truncate_names, event_name_width) for event in filtered_events)
...
if __name__ == "__main__":
    ...
    main(args.file_name, args.payload, args.show_participants, args.exclude_participants, args.queue, args.truncate_names)
